...
| `getBatchSQLRunningStateList()` | List\<SQLRunningState\> | 批量模式下获取每条 SQL 的独立上下文列表 |
| `getStatement()` | Statement | 获取底层 Statement 对象 |
| `getConnectionProxy()` | ConnectionProxy | 获取代理连接对象 |
| `getJdbcQuery()` | JdbcQuery | 获取 JDBC 查询工具 |
...
| `triggerDBMetaData(String tableName)` | void | 触发指定表的元数据订阅更新 |
...
| `getListeners()` | List\<SQLExecuteListener\> | 获取已注册的监听器列表 |
| `addListener(SQLExecuteListener)` | void | 注册监听器（去重 + 按 order 排序） |
| `addSystemListeners()` | void | 注册系统默认监听器（Insert/Update/Delete） |
...
- **2026-04-28**: 同步最新代码，新增 `SQLRunningState.getBatchSQLRunningStateList()`、`getStatement()`、`getConnectionProxy()`、`getJdbcQuery()`、`triggerDBMetaData(String)` 方法文档；新增 `SQLRunningContext.getListeners()`、`addSystemListeners()` 方法文档；Maven 坐标版本号更新为 1.0.18
