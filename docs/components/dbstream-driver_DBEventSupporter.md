...
| `DbColumn` | `com.codingapi.dbstream.scanner` | 列元数据（列名、类型、Java 类型、是否可空、注释、长度、是否主键） |
...
| `getPrimaryColumns()` | List\<DbColumn\> | 主键列的完整 DbColumn 对象列表 |
| `isPrimaryKey(String columnName)` | boolean | 判断指定列名是否为主键 |
| `getColumnByName(String column)` | DbColumn | 根据列名查找 DbColumn 对象 |
...
| `name` | String | 列名称 |
| `type` | String | 数据库字段类型 |
| `javaType` | Class\<?\> | Java 对应类型 |
| `nullable` | boolean | 是否可为空 |
| `comment` | String | 列注释 |
| `size` | int | 字段长度 |
| `primaryKey` | boolean | 是否为主键（由 DbTable 运行时回填） |
...
- **2026-04-28**: 同步最新代码，新增 `DbTable.getPrimaryColumns()`、`isPrimaryKey()`、`getColumnByName()` 方法文档；补充 `DbColumn` 完整 7 字段文档（含 `javaType`、`size`、`primaryKey`）；Maven 坐标版本号更新为 1.0.18
